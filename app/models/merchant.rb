class Merchant < ActiveRecord::Base
  ...
  def self.revenue_by_date_merchant(id, date)
    merchant = Merchant.find(id)
    {"revenue" => merchant.invoices.by_date(date).successful.joins(:invoice_items).sum("quantity * unit_price").to_s}
  end

  def self.total_revenue_merchant(id)
    merchant = Merchant.find(id)
    {"revenue" => merchant.invoices.successful.joins(:invoice_items).sum("quantity * unit_price").to_s}
  end
